...
public extension Flare {
    ...
    class func checkRenewSubscriptions(transactionList: [StoreTransaction], resultBlock: ((_ renewInfoList: [(RenewalInfo, StoreTransaction, RenewalState?)]?) -> Void)?) {
    ...
    @available(iOS 15.0, *)
    fileprivate class func checkRenewSubscriptions15AndUp(transactionList: [StoreTransaction], resultBlock: ((_ renewInfoList: [(RenewalInfo, StoreTransaction, RenewalState?)]?) -> Void)?) {
        Task {

            var list: [(RenewalInfo, StoreTransaction, RenewalState?)] = []
            for transaction in transactionList {
                ...
                case let .unverified(signedType, _):
                    ...
                    let item = (info, transaction, renewalState)
                    list.append(item)
    ...
    fileprivate class func checkRenewSubscriptions15Down(transactionList _: [StoreTransaction], resultBlock: ((_ renewInfoList: [(RenewalInfo, StoreTransaction, RenewalState?)]?) -> Void)?) {
