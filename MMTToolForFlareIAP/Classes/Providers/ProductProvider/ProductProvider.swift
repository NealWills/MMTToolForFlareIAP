        ...
        dispatchQueue.async {
            ...
            guard response.invalidProductIdentifiers.isEmpty else {
                self.dispatchQueueFactory.main().async {
                    ...
                    FlareLogger.error(message: L10n.Products.requestedProductsNotFound(response.invalidProductIdentifiers))
                }
                return
            }

            FlareLogger.debug(message: L10n.Products.requestedProductsReceived(response.products.map(\.productIdentifier)))
