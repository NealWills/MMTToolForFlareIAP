...
extension RefundRequestProvider: IRefundRequestProvider {
    ...
    func verifyTransaction(productID: String) async throws -> UInt64 {
        guard let state = await StoreKit.Transaction.latest(for: productID) else {
            FlareLogger.error(message: L10n.Purchase.transactionNotFound(productID))
        ...
        switch state {
            ...
            case let .unverified(_, result):
                FlareLogger.error(message: L10n.Purchase.transactionUnverified(productID, result.localizedDescription))
                throw result
        }
    }
        ...
        if let skError = error as? StoreKit.Transaction.RefundRequestError {
            switch skError {
                case .duplicateRequest:
                    FlareLogger.error(message: L10n.Refund.duplicateRefundRequest(skError.localizedDescription))
                    return .refund(error: .duplicateRequest)
                case .failed:
                    FlareLogger.error(message: L10n.Refund.failedRefundRequest(skError.localizedDescription))
